...
public class BarCodeService {
	...
	/**
	 * 利用barcode4j.jar生成条形码
	 * @param request
	 * @param response
	 * @throws ServletException
	 */
    public void createBarCode(HttpServletRequest request, HttpServletResponse response) throws ServletException {
        try {
...
//        	IJedisCacheManager jedisCacheManager = (IJedisCacheManager) BaseService
//        			.getBean("jedisCacheManager");
            String format = determineFormat(barCodeInfo);
            int orientation = 0;

            Configuration cfg = buildCfg(barCodeInfo);
            
            String msg = barCodeInfo.getMsg();
            if (msg == null) msg = "0123456789";
            ...
            if (bytes==null) {
                 ...
                 try {
                     if (format.equals(MimeTypes.MIME_SVG)) {
                         ...
                         String resText = barCodeInfo.getRes();
                         int resolution = 300; //dpi
                         if (resText != null) {
                             resolution = Integer.parseInt(resText);
                         }
                         ...
                         String gray = barCodeInfo.getGray(); 
                         BitmapCanvasProvider bitmap = ("true".equalsIgnoreCase(gray)
                             ? new BitmapCanvasProvider(
                                     bout, format, resolution,
                                     BufferedImage.TYPE_BYTE_GRAY, true, orientation)
            ...
            response.setHeader("Expires", nowCal.getTime().toString());
    ...
    protected String determineFormat(BarCodeInfo barCodeInfo) {
        String format = barCodeInfo.getFmt();
        format = MimeTypes.expandFormat(format);
        if (format == null) format = MimeTypes.MIME_GIF;
        return format;
    }
     ...
     * @todo Change to bean API
     */
    protected Configuration buildCfg(BarCodeInfo barCodeInfo) {
        DefaultConfiguration cfg = new DefaultConfiguration("barcode");
        //Get type
        String type = barCodeInfo.getType();
        if (type == null) type = "code128";
        DefaultConfiguration child = new DefaultConfiguration(type);
        cfg.addChild(child);
        //Get additional attributes
        DefaultConfiguration attr;
        String height = barCodeInfo.getHeight();
        if (height != null) {
            attr = new DefaultConfiguration("height");
            attr.setValue(height);
            child.addChild(attr);
        }
        String moduleWidth = barCodeInfo.getMw(); 
        if (moduleWidth != null) {
            attr = new DefaultConfiguration("module-width");
            attr.setValue(moduleWidth);
            child.addChild(attr);
        }
        String wideFactor = barCodeInfo.getWf();
        if (wideFactor != null) {
            attr = new DefaultConfiguration("wide-factor");
            attr.setValue(wideFactor);
            child.addChild(attr);
        }
        String quietZone = barCodeInfo.getQz(); 
        if (quietZone != null) {
            attr = new DefaultConfiguration("quiet-zone");
            if (quietZone.startsWith("disable")) {
                attr.setAttribute("enabled", "false");
        ...
        // where the human-readable has children for font name, font size, placement and
        // custom pattern.
        String humanReadablePosition = barCodeInfo.getHrp();
        String pattern = barCodeInfo.getHrpattern(); 
        String humanReadableSize = barCodeInfo.getHrsize();
        String humanReadableFont = barCodeInfo.getHrfont(); 
