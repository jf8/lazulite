...
//
		...
		userInfo.setLanguage(user.getLanguage().toString());
		userInfo.setHeadimgurl(user.getHeadimgUrl());
		userInfo.setNickname(stringFilter(user.getNickName()));
		...
		userInfo.setSubscribeTime(new Timestamp(Long.valueOf(user.getSubscribeTime())));
		return userInfo;
	}
