...
public class UserController extends BaseCRUDController<User, Long> {
    ...
    public String listAll(Pageable pageable, Model model) {
        return list(null, null, pageable, model);
    }
    ...
    public String list(
        ...
        if (organization != null && !organization.isRoot()) {
            //pageable.addSearchParam("organizationId", organization.getId());
        }
        if (job != null && !job.isRoot()) {
           /// pageable.addSearchParam("jobId", job.getId());
        }
    ...
    public Set<Map<String, Object>> autocomplete(
        ...
        return getUserService().findIdAndNames(pageable, term);
    }
